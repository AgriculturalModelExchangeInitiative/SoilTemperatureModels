...
CONTAINS
    ...
    SUBROUTINE doNetRadiation(solarRadn, &
        ...
        m1 = 0.
        TSTEPS2RAD = Divide(2.0 * cy_pi, REAL(ITERATIONSperDAY), 0.0)
        SOLARconst = 1360.0
        solarDeclination = 0.3985 * SIN((4.869 + (doy * 2.0 * cy_pi / 365.25)  &
                + (0.03345 * SIN((6.224 + (doy * 2.0 * cy_pi / 365.25))))))
        DO timestepNumber = 1 , ITERATIONSperDAY + 1-1, 1
            m1(timestepNumber+1) = (solarDeclination * SIN(latitude * cy_pi /  &
                    180.0) + (cD * COS(latitude * cy_pi / 180.0) * COS(TSTEPS2RAD *  &
    ...
    FUNCTION InterpTemp(time_hours, &
        ...
        IF(time .LT. min_t_time) THEN
            midnight_temp = SIN((0.0 + 0.25 - max_t_time) * 2.0 * cy_pi) *  &
        ...
        ELSE
            current_temp = SIN((time + 0.25 - max_t_time) * 2.0 * cy_pi) * (tmax  &
                    - tmin) / 2.0 + t2m
            res_cyml = current_temp
        END IF
        res_cyml = current_temp
