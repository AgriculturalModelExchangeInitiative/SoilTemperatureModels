...
def RadnNetInterpolate(internalTimeStep:float, solarRadn: float, cloudFr: float, cva: float, potE: float, potET: float, tMean: float, albedo: float, soilTemp: 'Array[float]') -> float:
    """
...
def main():
    ...
    with open("C:/Users/raihauti/Documents/CampbellOutputs.txt", "w") as file:
        ...
        (soilTemp, minSoilTemp, maxSoilTemp, aveSoilTemp, morningSoilTemp, newTemperature, maxTempYesterday, 
         minTempYesterday, thermalCondPar1, thermalCondPar2, 
        thermalCondPar3, thermalCondPar4, thermalConductivity, thermalConductance, 
        heatStorage, volSpecHeatSoil, thickness, depth, bulkDensity, soilW, clay, rocks,
       carbon, sand, silt) = init_campbell(numberLayer, THICK, SLBDM, 
                                                T2M[0], TMAX[0], TMIN[0], TAV, TAMP, XLAT, CONSTANT_TEMPdepth,
                                                SLOC, SLCLY, SLROCK, SLSILT, SLSAND, soilW, doy[0], 
                                                canopyHeight, albedo, SRAD[0], ESP[0], 0.0, 
                                                EOAD[0], instrumentHeight)
        ...
        rocks, carbon, sand, silt, airPressure) = model_campbell(numberLayer, thickness, depth, CONSTANT_TEMPdepth, bulkDensity, 
                                                T2M[i], TMAX[i], TMIN[i], TAV, TAMP, XLAT, 
                                                clay, soilW, doy[i], airPressure, 
                                                canopyHeight, albedo, SRAD[i], ESP[i], 0.0, 
                                                EOAD[i], soilTemp, newTemperature, minSoilTemp, maxSoilTemp, aveSoilTemp,
                                                ...
                                                netRadiationSource, windSpeed, carbon, rocks, silt, sand, boundaryLayerConductance) 
