...
namespace Models.Soils.SoilTemp
{
    ...
    public class SoilTemperature : Model, ISoilTemperature
    {
        ...
        {
            double MWair = 0.02897;     // molecular weight air (kg/mol)
            double RGAS = 8.3143;       // universal gas constant (J/mol/K)
            double HPA2PA = 100.0;      // hectoPascals to Pascals
        ...
        private void getProfileVariables()
        {
            ...
            if (waterBalance.SW != null)
            {
                for (int layer = 1; layer <= numLayers; layer++)
                    soilWater[layer] = Divide(waterBalance.SW[layer - 1]*thickness[layer-1], thickness[layer], 0);
                for (int layer = numLayers + 1; layer <= numLayers + numPhantomNodes; layer++)
        ...
        private void doProcess()
        {
            ...
            {
                timeOfDaySecs = internalTimeStep * Convert.ToDouble(timeStepIteration);  // CHANGE CYML system.convert to convert
        ...
        private void doVolumetricSpecificHeat()
        {
            ...
            mapLayer2Node(volspecHeatSoil_, ref volSpecHeatSoil);
        }
                ...
                case "eos":
                    {
                        ...
                        radnNet = Divide(waterBalance.Eos * latentHeatOfVapourisation, timestep, 0);
                        break;
                    }
            }

            // get latent heat flux, LE (W/m)
            double latentHeatFlux = Divide(waterBalance.Es * latentHeatOfVapourisation, timestep, 0);
        ...
        private double interpolateNetRadiation(double solarRadn, double cloudFr, double cva)
        {
            ...
            double PenetrationConstant = Divide(Math.Max(0.1, waterBalance.Eos),
        ...
        {
            double precisionMargin = 0.00001;          // margin for precision err of lower
