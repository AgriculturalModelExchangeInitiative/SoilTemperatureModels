...
void layers_temp::Calculate_Model(soil_tempState &s, soil_tempState &s1, soil_tempRate &r, soil_tempAuxiliary &a, soil_tempExogenous &ex)
{
    ...
    s.layer_temp = std::vector<double> (layers_nb);
    ...
    for (z=1 ; z!=layers_nb + 1 ; z+=1)
    {
        s.layer_temp[z - 1] = std::accumulate(s.temp_profile.begin() + (up_depth[z - 1] + 1 - 1),s.temp_profile.begin() + up_depth[(z + 1 - 1)], 0.0) / layer_thick[(z - 1)];
    }
}
