import numpy 
from math import *
def model_soiltemperaturecampbell(float DepthConstantSoilTemperature,
                                  float Timestep,
                                  float HeatCapacity[['array', 'double']],
                                  float NetRadiation,
                                  float SurfaceSoilTemperature,
                                  float SoilTemperatureByLayers[['array', 'double']],
                                  float BottomTemperature,
                                  float BulkDensity[['array', 'double']],
                                  float ETA,
                                  float Clay[['array', 'double']],
                                  float ThermalK,
                                  float VolumetricWaterContent[['array', 'double']],
                                  float TemperatureConductance[['array', 'double']],
                                  float SurfaceTemperatureMaximum,
                                  float LayerThickness[['array', 'double']]):
    """

    SoilTemperatureCampbell model
    Author: simone.bregaglio@unimi.it
    Reference: ('http://bioma.jrc.ec.europa.eu/ontology/JRC_MARS_biophysical_domain.owl',)
    Institution: University Of Milan
    ExtendedDescription: Campbel strategy - strategy. References: Soil physics with basic Gaylon S. Campbell Elsevier 1985.
    ShortDescription: None

    """
    cdef float SoilTemperatureByLayersChange[['array', 'double']]
    ...
    return  HeatCapacity, SoilTemperatureByLayersChange, SoilTemperatureByLayers, LatentHeatFlux, TemperatureConductance
