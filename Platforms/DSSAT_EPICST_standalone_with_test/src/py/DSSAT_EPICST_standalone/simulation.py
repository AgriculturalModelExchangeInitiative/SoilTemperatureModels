...
def simulation(datafile, vardata, params, init):
    ...
    #outputs
    output_names = ["SRFTEMP","NDays","TDL","WetDay","ST","TMA","X2_PREV"]
    ...
    for i in range(0,len(df.index)-1):
        ...
        TAMP = t_TAMP[i]
        SRFTEMP = t_SRFTEMP[i]
        SRFTEMP,NDays,TDL,WetDay,ST,TMA,X2_PREV= STEMP_EPIC_Component.model_stemp_epic_(DS,TAVG,TAV,ISWWAT,CUMDPT,DSMID,LL,DLAYR,BD,TMAX,TMA,SW,MULCHMASS,NLAYR,SNOW,BIOMAS,NDays,WetDay,TDL,NL,DEPIR,DUL,TMIN,ST,X2_PREV,RAIN,TAMP,SRFTEMP)

        df_out.loc[i] = [SRFTEMP,NDays,TDL,WetDay,ST,TMA,X2_PREV]
